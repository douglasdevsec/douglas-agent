...
//! Electron app's APP_ROOT/../.. trick, because Douglas-Setup.exe is meant
...
///
/// `pin` is the commit-or-branch from either Douglas-Setup's build-time
...
async fn download(kind: ScriptKind, commit_or_ref: &str, dest_path: &Path) -> Result<()> {
    let url = format!(
        "https://raw.githubusercontent.com/douglasdevsec/douglas-agent/{}/scripts/{}",
        commit_or_ref,
        kind.filename()
    );
    ...
    let response = reqwest::Client::builder()
        ...
        .get(&url)
        .header("User-Agent", "douglas-setup/0.0.1")
        .send()
        .await
