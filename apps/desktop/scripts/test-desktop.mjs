...
// no longer bundles the Douglas Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Douglas Agent.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Douglas Agent'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Douglas Agent.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'Douglas Agent'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `DouglasAgent-${PACKAGE_JSON.version}-mac-${ARCH}.dmg`)
  }

  const prefix = `DouglasAgent-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `DouglasAgent-${PACKAGE_JSON.version}-mac-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'DouglasAgent-${version}-${os}-${arch}.${ext}'
...
//   - The Douglas Agent Python payload is NOT shipped (it's fetched at first
