#!/usr/bin/env node
// set-exe-identity.mjs — stamp the Douglas Agent icon + version metadata onto
// the built Douglas Agent.exe using rcedit, completely decoupled from
// electron-builder's signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Douglas Agent.exe keeps the stock Electron icon
// and "Electron" taskbar name. This script restores the icon + identity by calling
...
// so EVERY packed build — first install, `douglas desktop`, the installer's
...
//   node scripts/set-exe-identity.mjs <path-to-Douglas Agent.exe>
//
...
// Stamp the Douglas Agent icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = resolve(import.meta.dirname, '..')) {
  ...
  await rcedit(exe, {
    icon,
    'version-string': {
      ProductName: 'Douglas Agent',
      FileDescription: 'Douglas Agent',
      CompanyName: 'DouglasDevSec',
      LegalCopyright: 'Copyright (c) 2026 DouglasDevSec'
    }
  })

  console.log('[set-exe-identity] done — Douglas Agent icon + identity stamped')
}
