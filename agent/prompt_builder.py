...
def computer_use_guidance(platform_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "when Douglas has none; explicit Douglas YOLO uses a private unrestricted "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Douglas "
...
PLATFORM_HINTS = {
    ...
    "tui": (
        "You are running in the Douglas terminal UI (TUI). "
        "Cron jobs scheduled from this session are LOCAL-ONLY: their output is "
        "saved (viewable via cronjob action='list') but is NOT delivered back "
    ...
    "desktop": (
        "You are chatting inside the Douglas Agent desktop app — a graphical chat "
        ...
        "Built-in Social panel: the left sidebar has a 'Social' nav item that "
        "opens a panel for connecting Facebook, Instagram, YouTube, LinkedIn, "
        "TikTok, and X so Douglas Agent can publish content on the user's "
        "behalf. Today, Facebook is the only network with a real, working "
        "connection — Instagram/YouTube/LinkedIn/TikTok/X are visible in the "
        "panel but not yet functional. To connect Facebook: click 'Social' in "
        "the sidebar, click Connect next to Facebook, then follow the wizard — "
        "it asks for the user's OWN Meta Developer app's App ID and App "
        "Secret (never a Douglas-owned one), then the Facebook Page ID the "
        "user administers, then opens the browser to authorize. When asked "
        "what the social/RRSS buttons are for or how to connect a network, "
        "this is the real, current answer — don't guess at plugins, "
        "extensions, themes, or other apps."
    ),
    "sms": (
    ...
    "webui": (
        "You are in the Douglas WebUI, a browser-based chat interface. "
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Douglas itself is running. The host OS, home, and cwd "
                f"of the Douglas process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Douglas "
...
def build_skills_system_prompt(
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Douglas Agent itself — its CLI, config, models, providers, tools, "
